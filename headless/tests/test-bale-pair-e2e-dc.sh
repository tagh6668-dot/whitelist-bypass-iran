#!/bin/sh
# DC tunnel-mode e2e: spawn headless-bale-creator, feed its join_link to
# anonymous headless-bale-joiner with --tunnel-mode dc, then verify SOCKS5
# reaches the public internet and measure throughput.
#
# Usage:
#   ./test-bale-pair-e2e-dc.sh [path/to/bale-cookies.json]
...
C_LOG=$(mktemp -t bale-c-dc.XXXXXX.log)
J_LOG=$(mktemp -t bale-j-dc.XXXXXX.log)
C_PID=""
J_PID=""
cleanup() {
...
echo "=== joiner (tunnel-mode dc) ==="
"$JOINER" --join-link "$JOIN_LINK" --socks-port "$SOCKS_PORT" --tunnel-mode dc > "$J_LOG" 2>&1 &
J_PID=$!
...
if ! grep -q "tunnel mode selected: dc (role=joiner)" "$J_LOG"; then
    echo "FAIL: joiner did not select dc mode" >&2
    tail -40 "$J_LOG" >&2
    exit 1
fi
echo "joiner: dc mode selected"
...
if ! grep -q "tunnel mode selected: dc (role=creator)" "$C_LOG"; then
    echo "FAIL: creator did not switch to dc mode after joiner traffic" >&2
    tail -40 "$C_LOG" >&2
    exit 1
fi
echo "creator: dc mode selected"
